...
public:
    ...
    SingleLinkedList(const SingleLinkedList& other) {
        ...
        for (auto it = other.cbegin(); it != other.cend(); ++it) {
            // Создаём новый узел и добавляем его в конец временного списка
            if (tmp.head_.next_node == nullptr) {
                tmp.head_.next_node = new Node(*it, nullptr);
                current = tmp.head_.next_node;
            } else {
    ...
    Iterator InsertAfter(ConstIterator pos, const Type& value) {
        assert(pos != end());
        Node* new_node = new Node(value, pos.current_node_->next_node);
        pos.current_node_->next_node = new_node;
        ++size_;
    ...
    Iterator EraseAfter(ConstIterator pos) noexcept {
        assert(pos != end());
        Node* node_to_delete = pos.current_node_->next_node;
        if (node_to_delete) {
            pos.current_node_->next_node = node_to_delete->next_node;
            delete node_to_delete;
            --size_;
        }
        return Iterator{pos.current_node_->next_node};
    }
